...
foreach( $results as &$result ) :
    ...
    if( $result->wardid == $searched_location['ward']->wardid )   // modified from wardnumber
    ...
    <h2 class="ward-name"><?php print sprintf('%s', $result->wardname); ?></h2>
    <?php endif; endif; ?>
    ...
    <div class="result <?php print $class; ?>" id="ward<?php print !empty($result->wardid) ? ('-' . $result->wardid) : ''; ?>">
        ...
        <div class="indiv-details">
            ...
            <?php if( !empty($result->email) ) : ?>
				<?php /* print sprintf('<h3>Email: %s</h3>',strpos($result->email, "http://")); */  //Modified 1/29/2012 to account for http forms in email links ^NikG?> 
				<?php if(strpos($result->email, "http://") !== false) : ?>
					<div class="email"><span class="lbl">Email:</span> <?php print sprintf('<a href="%s" title="Send an email to %s %s" target="_blank">link</a>', $result->email, $result->firstname, $result->lastname); ?>
					</div>
				<?php else : ?>
